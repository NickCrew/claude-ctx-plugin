...
class DependencyGraph:
    ...
    @staticmethod
    def create_tree_lines(root_name: str, dependencies: Dict[str, Any], max_depth: int = 3) -> List[str]:
        ...
        DependencyGraph._add_dependency_lines(lines, dependencies, level=1, max_depth=max_depth)
        return lines
...
class StatsDisplay:
    ...
    def create_stats_bar(stats: Dict[str, Any]) -> str:
        ...
        if 'warnings' in stats and stats['warnings']:
            parts.append(f"[yellow]{Icons.WARNING} {stats['warnings']} warnings[/yellow]")

        return " [dim]│[/dim] ".join(parts) if parts else "[dim]No stats available[/dim]"
